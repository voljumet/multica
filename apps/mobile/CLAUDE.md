...
4. **Header is the iOS-native nav header**, never body-drawn. Every formSheet sheet sets `headerShown: true` + `title` in its Stack.Screen options (spread over SHEET_OPTIONS, whose `headerShown: false` is just the base value), and the body renders no title row of its own. A body-drawn title paints under the native grabber — the react-native-screens #3634 overlap class — so the native header is the standard for every sheet. Sheets with right-side actions (Reset / Clear / Done) keep them as a slim action row at the top of the body, below the native header.
