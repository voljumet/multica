...
从 GitHub、ClawHub 或 Skills.sh 导入的 Skill 会保存来源 URL（`config.origin.source_url`），内容变更时会更新 `updated_at`。在 Skills 列表和详情页，创建者或工作区管理员可以看到 **从 URL 更新**——一点即可重新拉取远程内容，**保留 Skill ID 与智能体绑定**，并覆盖正文与附属文件。手动创建或从本地运行时导入的 Skill 没有远程来源，因此不会显示该操作。
