...
## Updating a URL-imported skill

When a skill is imported from GitHub, ClawHub, or Skills.sh, Multica stores the source URL on the skill (`config.origin.source_url`) and bumps `updated_at` whenever content changes. On the skills list and the skill detail page, creators and workspace admins see **Update from URL** — one click re-fetches the remote skill and replaces local content and supporting files while **keeping the skill ID and agent attachments**. Manual and runtime-local skills have no remote source, so they do not show this action.
